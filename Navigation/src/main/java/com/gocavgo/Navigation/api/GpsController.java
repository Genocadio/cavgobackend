...
public class GpsController {
    ...
    private ResponseEntity<TripResponse> processBatchGpsUpdates(List<GpsUpdateRequest> updates) {
        ...
        try {
            ...
            // Check for route update (rerouting)
            if (result.route != route) {
                log.info("Reroute detected for tripId: {}, updating trip route in database", trip.getId());
                tripService.updateTripRoute(trip.getId(), result.route);
            }
            ...
            if (result.waypointProgresses != null && !result.waypointProgresses.isEmpty()) {
                if (isTripCompleted(result.waypointProgresses)) {
                    // Mark trip as completed and return the completed trip payload
                    tripService.updateTripStatus(carId, TripStatus.COMPLETED);
                    TripResponse completedResponse = tripService.getTripById(trip.getId(), false);
