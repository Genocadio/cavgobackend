...
| `COMPLETED` | All waypoints have been reached (final waypoint is ARRIVED or DONE). On the GPS batch that completes the trip, the API returns 200 with the completed trip payload. Subsequent `/api/gps` calls for the car return 404 (no active/created trip). |
...
5. The GPS batch that detected completion returns `200 OK` with the completed trip
6. A RabbitMQ event is published to the fanout exchange `navigation.trip.update` with the same payload as the REST response
...
Response: TripResponse (200 OK). If the batch completes the trip, the response contains `status=COMPLETED` and `completedAt`. If no `CREATED`/`ACTIVE` trip exists for the `carId`, returns `404 Not Found`. On completion, the same payload is also published to RabbitMQ exchange `navigation.trip.update` (fanout).

### RabbitMQ Events
- Exchange: `navigation.trip.update` (fanout, durable)
- Trigger: Fired when a trip transitions to `COMPLETED` during GPS processing
- Payload: Identical to the REST `TripResponse` for the completing batch (includes `trip`, `currentLocation`, `instructions` if present)

#### What Consumers Should Expect
- Protocol: AMQP 0-9-1 via RabbitMQ fanout exchange. No routing key is required (empty string is used on publish).
- Message format: JSON produced by Jackson, matching the REST `TripResponse` schema.
- Delivery: Exchange is durable; bind your own durable queue(s) to receive events. Publisher does not declare queues.
- Ordering: Messages are emitted only on completion; one message per completed trip.
- Retry: Publisher best-effort; failures are logged but not retried. Consumers should handle occasional misses.
...
```json
{
  "trip": {
    ...
    "carId": "vehicle-001",
    "status": "COMPLETED",
    "completedAt": "2025-12-19T10:30:00Z",
    "waypoints": [...],
    "waypointProgresses": [...],
    "includeOrigin": false,
    "isCityTrip": false,
...
Override via environment variables: `RABBITMQ_HOST`, `RABBITMQ_PORT`, `RABBITMQ_USERNAME`, `RABBITMQ_PASSWORD`.
...
Behavior:
- Deletes the trip record from the database
- Deletes all navigation snapshots associated with the trip
- Clears Redis navigation state for the trip's `carId`

Effects:
- After deletion, the system no longer tracks that car; subsequent GPS updates for that `carId` return `404 Not Found` until a new trip is created
- Trip creation purges any previous trips for the same `carId` and clears Redis, ensuring only one active trip per car
```
