 ...
 *
 * <ul>
 *   <li>{@link #getUser(Long)} — read a user's profile + roles</li>
 *   <li>{@link #updateUserRoles(Long, List)} — assign roles; the next issued
 *       tokens for that user carry the new roles</li>
 *   <li>{@link #setUserEnabled(Long, boolean)} — enable/disable an account</li>
 *   <li>{@link #listRoles()}, {@link #createRole(...)}, {@link #updateRole(...)},
 *       {@link #deleteRole(Long)} — manage the organisation's role definitions</li>
 * </ul>
 *
 * <p>Users are NOT created by the backend — the Android/web apps register them
 * directly against Nexxauth's public auth endpoints. This client only reads,
 * updates roles on, or disables existing users.
 *
 * <p>Every call is scoped to the organisation from configuration and requires
 * the SERVER client's static token plus its {@code X-Client-Id}.
 */
@Component
public class NexxauthClient {
    ...
    // ── Role management (organisation role definitions) ───────────────────────
    ...
    /** User as returned by the Nexxauth org API (role names only, no permissions). */
    ...
    /** Role definition as returned by the Nexxauth org API (name + permissions). */
