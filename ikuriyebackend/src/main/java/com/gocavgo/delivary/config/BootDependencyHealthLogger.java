 ...
 * ready, so it can never block or break startup. For every dependency
 * ikuriyebackend talks to, logs whether it is reachable — and the failure
 * reason when it is not — so health on boot is visible in the logs instead of
 * silently skipped.
 *
...
public class BootDependencyHealthLogger {
    ...
    private final String cavgomainBaseUrl;
    ...
    public BootDependencyHealthLogger(
            ...
            @Value("${cavgomain.base-url:}") String cavgomainBaseUrl,
        ...
        this.cavgomainBaseUrl = normalize(cavgomainBaseUrl);
    ...
    public void onReady() {
        log.info("════════════════ IKURIVE BOOT DEPENDENCY HEALTH ════════════════");
        log.info("  Targets: RabbitMQ, PostgreSQL, NeXXauth, Supabase, CavgoMain");
        ...
        check("Supabase", supabaseUrl, "supabase.url",
                () -> httpReachable(supabaseUrl));
        check("CavgoMain", cavgomainBaseUrl, "cavgomain.base-url",
                () -> httpReachable(cavgomainBaseUrl));
        log.info("═══════════════════════════════════════════════════════════════");
    }
    ...
    private boolean httpReachable(String baseUrl) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder(uri)
                    ...
                    .header("User-Agent", "ikuriyebackend-boot-health")
                    .build();
